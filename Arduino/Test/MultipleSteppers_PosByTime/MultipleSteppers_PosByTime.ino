...
AccelStepper stepper1 (AccelStepper::DRIVER, lyStep, lyDir);
...
void loop() {
  ...
  if (stepper1.distanceToGo() == 0 && isNew) {
    stepper1.setMaxSpeed(MAX_SPEED);
    stepper1.setAcceleration(computeAcceleration(abs(stepper1.currentPosition() - moveToPos), timeToMove));
    stepper1.moveTo(moveToPos);
    newTwoTimes++;
    if (newTwoTimes >= 2) {
